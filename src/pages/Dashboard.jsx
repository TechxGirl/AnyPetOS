...
function getMetricValue(
  ...
  const values = {
    adoption: pets.filter((pet) =>
      ["Adoption Ready", "Available"].includes(pet.status)
    ).length,
    attention: attentionPets.length,
    intake: pets.filter((pet) => ["Intake", "New"].includes(pet.status)).length,
    logs: pets.reduce(
      (total, pet) => total + (pet.logs?.length || 0),
      0
    ),
    meds: dueMeds.length,
    modules: "Ready",
    pipeline: pets.length,
    ready: pets.filter((pet) =>
      ["Healthy", "Ambassador", "Available", "Sale Ready"].includes(pet.status)
    ).length,
...
function countPipelinePets(pets, pipeline) {
  return pets.filter((pet) =>
    pipeline.statuses.some(
      (status) =>
        String(pet.status || "").toLowerCase() === status.toLowerCase()
    )
  ).length;
}
...
function DashboardAnimalTile({ pet, openProfile }) {
  ...
  return (
    ...
    >
      ...
      <span
        className={`premiumMiniBadge status-${String(
          pet.status || "healthy"
        )
          .toLowerCase()
          .replace(/\s+/g, "-")}`}
...
export default function Dashboard({
  ...
  const transferPets = pets.filter(
    (pet) => pet.transfer?.status === "pending" || pet.share?.enabled
  );
  ...
  const statCards = [
    {
      label: workspace.terminology.collection,
      value: pets.length,
      icon: workspace.icon,
      hint: `${favoritePets.length} favorites`,
    },
  ...
  if (pets.length === 0) {
    return (
      <div className="feed premiumDashboardPage">
        <section
          className="premiumDashboardHero"
    ...
    <div className="feed premiumDashboardPage">
      ...
      >
        <div>
          ...
          <p>
            {workspace.focusTitle}: {workspace.description}
          </p>
      ...
      <div className="premiumStatStrip">
        {statCards.map((card) => (
          <Card
            ...
            style={{ "--workspace-card-accent": workspace.accent }}
          >
            <span className="premiumStatIcon">
      ...
      <div className="premiumDashboardTwoColumn">
        ...
        <Card className="premiumReminderCard">
          ...
          {todayFocus.length === 0 ? (
            <p className="premiumCalmText">
              Everything looks calm today. No urgent care tasks right now.
            </p>
          ) : (
        ...
        <div className="workspaceQuickActions premiumQuickActions">
          {workspace.quickActions.map((action) => (
            <Button
              key={action.label}
              variant="outline"
              leftIcon={<Icon name={action.icon} size={16} />}
