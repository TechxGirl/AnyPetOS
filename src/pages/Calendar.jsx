...
export default function Calendar({ pets, giveMedication }) {
  ...
  const feedEvents = pets.flatMap((pet) =>
    (pet.logs || [])
      .filter(
        (log) =>
          log.type === "Fed" &&
  ...
  const medicationEvents = getMedicationCalendarEvents(
    pets,
    rangeStart,
    rangeEnd
  );
  ...
  const filteredEvents = [
    ...
    .filter(
      (event) =>
        animalFilter === "all" ||
        String(event.petId) === animalFilter
    )
    .filter(
      (event) =>
        categoryFilter === "all" ||
  ...
  const recordDose = async (options) => {
    ...
    try {
      ...
      await giveMedication?.(
        recordTarget.pet.id,
        recordTarget.medication.id,
        options
      );
    ...
    <div className="feed careCalendarPage">
      ...
      <Card>
        ...
        <div
          ...
          aria-label="Medication status legend"
        >
          {[
            "given",
            "given-late",
            ...
            <Badge
              key={status}
              variant={getMedicationStatusVariant(
                status
              )}
            >
              {getMedicationStatusLabel(
                ...
                {events
                  ...
                  .map((event) =>
                    ...
                    "medication" ? (
                      <div
                        key={event.id}
                        className={`calendarEvent calendarEvent--${event.status}`}
                      >
                        <span>
                          {event.petName}
                        </span>
