# 💡 AnyPetOS Ideas Parking Lot

This file exists so good ideas never get lost, but they also do not derail the current phase.

## Rule

If an idea does not help the current phase, it goes here.

We are not saying no.
We are saying not yet.

---

## Future Ideas

- Birthday messages
- Badges and achievements
- Home screen widgets
- AI photo identification
- Morph identification
- NFC tap sharing
- Marketplace
- Community feed
- Breeder verification
- Local expo finder
- Smart enclosure builder
- Food and supply inventory
- Financial tracking for breeders
- Public breeder profiles
- Rescue directory
- Weather alerts
- Printable cage cards
- Emergency evacuation lists

---

## Reminder

Current mission comes first.
Ideas go here.
Then we return to the sprint.