          ...
          meds.map((med) => {
            ...
            return (
              <div key={med.id} className={`card innerCard ${due ? "overdue" : ""}`}>
                ...
                <p><strong>Last given:</strong> {getMedicationLastGiven(med) ? new Date(getMedicationLastGiven(med)).toLocaleString() : "Not logged"}</p>
                <p><strong>Next dose:</strong> {nextDose ? new Date(nextDose).toLocaleString() : "Due now"}</p>
                {med.notes && <p><strong>Notes:</strong> {med.notes}</p>}
                <div className="buttonRow">
                  <Button loading={saving} leftIcon={<Icon name="check" size={16} />} onClick={() => giveMedication(pet.id, med.id)}>Record dose now</Button>
