...
export default function MedicationPanel({
  ...
  const validateForm = () => {
    ...
    if (
      ...
      nextErrors.durationDays = "Enter a number of days greater than zero.";
    }
  ...
  const startEditMed = (petId, med) => {
    ...
    const firstDoseDate = formatLocalDateInput(med.firstDose);
      ...
      const updatedMeds = (pet.meds || []).map((med) =>
        med.id === editingMed.medId
          ? {
              ...
              firstDose: firstDoseTimestamp || med.firstDose || null,
              startDate: firstDoseTimestamp || med.startDate || Date.now(),
              lastGiven: med.lastGiven || null,
              doseHistory: Array.isArray(med.doseHistory)
                ? med.doseHistory
                : [],
      ...
      <Card className="medication-form-card">
        ...
        >
          <div className="medication-form-grid">
            ...
            <FormField label="Course duration (days)" error={errors.durationDays}>
              {(fieldProps) => (
                <Input
                  {...fieldProps}
                  type="number"
                ...
                return (
                  <Card key={`${pet.id}-${med.id}`} className="medication-course-card">
                    ...
                    <dl className="medication-meta-grid">
                      ...
                      <div>
                        <dt>Last given</dt>
                        <dd>{formatDateTime(getMedicationLastGiven(med))}</dd>
                      </div>
                      <div>
                        <dt>Next dose</dt>
