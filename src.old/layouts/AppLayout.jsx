...
export default function AppLayout({ sidebar, children }) {
  ...
  return (
    <div className={["pp-app-shell", navigationOpen ? "is-navigation-open" : ""].filter(Boolean).join(" ")}>
      <button
        type="button"
      ...
      <div className="pp-app-shell__sidebar" onClick={(event) => {
        if (event.target.closest(".pp-sidebar__item")) setNavigationOpen(false);
      }}>
        {sidebar}
      </div>
      ...
      <div className="pp-app-shell__content">
        <header className="pp-app-shell__mobile-header">
          ...
          <IconButton
            ...
            icon={<Icon name={navigationOpen ? "close" : "menu"} size={21} />}
