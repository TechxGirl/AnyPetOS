  ...
  return (
    <aside className="pp-sidebar" aria-label="Main navigation">
              ...
              {group.items.map((item) => {
                ...
                return (
                  <button
                    key={item.page}
                    type="button"
                    className={["pp-sidebar__item", active ? "is-active" : ""].filter(Boolean).join(" ")}
      ...
      <footer className="pp-sidebar__footer">
        <p className="pp-sidebar__footer-note">Care records, schedules, and health details in one organized workspace.</p>
      </footer>
    </aside>
  );
}
